...
// 版本号、初始化、渠道
/// <#Description#>
@interface SobotTMService : NSObject
...
+(void)getCallOutUserInfoByNumber:(NSString *) number start:(void (^)(NSString * _Nonnull))startBlock success:(NetworkResultBlock)successBlock failed:(NetworkResultBlock)failedBlock;
...
/// 提交跟进记录 callID必传  备注 和 客户ID  有就传没有就不传或者传空
/// - Parameters:
///   - remark: 备注
///   - callID: 通话ID
///   - customerId: 客户ID
...
///   - successBlock: successBlock description
///   - failedBlock: failedBlock description
+(void)postFollowUps:(NSString *)remark
              callID:(NSString *)callID
...
///   - successBlock: successBlock description
///   - failedBlock: failedBlock description
+(void)getCdrRemark:(NSString*)callID
         startBlock:(void(^)(NSString *urlString)) startBlock
            success:(__nullable NetworkResultBlock )successBlock
             failed:(__nullable NetworkResultBlock )failedBlock;
@end
